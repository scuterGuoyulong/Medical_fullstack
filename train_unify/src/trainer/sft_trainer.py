...
class QwenSFTTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            try:
                ...
                logger.warning(
                    "Generation-based sample eval failed at global_step=%s. "
                    "This is often caused by model.generate incompatibility with DeepSpeed ZeRO/offload. "
                    "Skipping sample predictions and continuing training. Error: %r",
                    self.state.global_step,
                    exc,
                )
                self._maybe_save_sample_eval_error(exc, metric_key_prefix)
                metrics = {}
                if all_losses:
        ...
        self._maybe_save_sample_eval_predictions(
            all_predictions,
            all_references,
            metric_key_prefix,
        )
    ...
    def _maybe_save_sample_eval_predictions(
        self,
        predictions: List[str],
        references: List[str],
        metric_key_prefix: str,
    ) -> None:
        if not getattr(self.args, "sample_eval_save_predictions", False):
            return
        if not self.is_world_process_zero():
            return

        output_dir = os.path.join(self.args.output_dir, "sample_eval_predictions")
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(
            output_dir,
            f"{metric_key_prefix}_step_{self.state.global_step}.json",
        )
        rows = [
            {
                "index": idx,
        ...
        logger.info(f"Saved sample eval predictions to {output_path}")

    def _maybe_save_sample_eval_error(self, exc: Exception, metric_key_prefix: str) -> None:
        if not getattr(self.args, "sample_eval_save_predictions", False):
            return
        if not self.is_world_process_zero():
            return
        ...
        output_path = os.path.join(
            output_dir,
            f"{metric_key_prefix}_step_{self.state.global_step}_error.json",
        )
        payload = {
            ...
            "note": (
                "Generation-based eval failed inside the DeepSpeed training process. "
                "Training can continue; run sample generation after saving/merging a checkpoint if needed."
            ),
        }
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)
        logger.info(f"Saved sample eval error to {output_path}")
