...
```bash
source /home/notebook/data/group/guoyulong/code/image_enhance/vlm-prx/SuperResolution_train_prx/med_omni_stack/medical_fullstack/use_group_data_paths.sh
```
...
| 变量 | 默认含义 |
|:--|:--|
| `MED_OMNI_STACK_ROOT` | `med_omni_stack` 根目录 |
...
推荐把代码、数据和模型放在同一个 `med_omni_stack` 根目录下，路径变量和脚本默认都按这个布局解析：
...
```bash
cd /path/to/med_omni_stack

# 如果路径与本项目默认路径一致，可直接 source。
source medical_fullstack/use_group_data_paths.sh

# 新机器路径不同，更推荐在当前 shell 覆盖。
export MED_OMNI_STACK_ROOT=/path/to/med_omni_stack
export MEDICAL_FULLSTACK="${MED_OMNI_STACK_ROOT}/medical_fullstack"
export MEDICAL_DS="${MED_OMNI_STACK_ROOT}/DataSets/medical"
export MIMIC_CXR_ROOT="${MED_OMNI_STACK_ROOT}/DataSets/mimic-cxr-dataset"
export MODEL_NAME_OR_PATH="${MED_OMNI_STACK_ROOT}/models/models/Qwen/Qwen3___5-4B"
...
export MIMIC_JPEG_DIR="${MED_OMNI_STACK_ROOT}/DataSets/mimic-cxr-jpeg-sample200"
...
export IMAGE_FOLDER="${MED_OMNI_STACK_ROOT}/DataSets/mimic-cxr-jpeg-sample200"
...
deepspeed --num_gpus=1 src/train/train_sft.py \
  ...
  --image_folder "${MED_OMNI_STACK_ROOT}/DataSets/mimic-cxr-jpeg-sample200" \
...
```bash
cd "${MEDICAL_FULLSTACK}/train_unify"
export DATA_JSON="${MED_OMNI_STACK_ROOT}/DataSets/medical/mixed_sft/train_qa_report_qwen_vl.json"
export IMAGE_FOLDER="${MED_OMNI_STACK_ROOT}/DataSets/mimic-cxr-jpeg-sample200"
bash scripts/finetune_qwen35_4b_local_medical_sft.sh
```
...
```bash
cd "${MEDICAL_FULLSTACK}/train_unify"
export SFT_JSON="${MED_OMNI_STACK_ROOT}/DataSets/medical/mixed_sft/train_qa_report_qwen_vl.json"
export IMAGE_FOLDER="${MED_OMNI_STACK_ROOT}/DataSets/medical/mixed_sft"
export VL_MODEL_PATH="${PWD}/output/qwen35_4b_medical_sft"
export OUT_JSON="${PWD}/output/medical_dpo_patient_judged.json"
bash scripts/build_medical_dpo_patient_judge.sh
```
